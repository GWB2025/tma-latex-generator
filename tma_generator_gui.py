...
class ToolTip:
    ...
    def on_enter(self, event: Optional[tk.Event] = None) -> None:
        ...
        label = tk.Label(tooltip, text=self.text,
                         justify='left', **TOOLTIP_STYLE)
...
class HelpDialog:
    ...
    def _create_help_content(self) -> None:
        ...
        title_label = ttk.Label(
            main_frame, text="TMA LaTeX Generator - User Guide", font=("Arial", 14, "bold"))
        title_label.pack(pady=(0, 10))
        text_widget = scrolledtext.ScrolledText(
            main_frame, wrap=tk.WORD, font=("Arial", 10), height=30)
...
class LaTeXFileGenerator:
    ...
    def create_directory(self, directory: str) -> str:
        ...
        try:
            ...
            print(
                f'Directory {directory_path} exists, renaming to {backup_path}')
    ...
    def create_main_tex_file(self, folder: str, basename: str, number_of_questions: int) -> None:
        ...
        try:
            content = self._generate_main_tex_content(
                basename=basename, number_of_questions=number_of_questions)
    ...
    def _generate_main_tex_content(self, basename: str, number_of_questions: int) -> str:
        ...
        lines.append("")
        include_files = [
            f"{QUESTION_PREFIX}{i+1}" for i in range(number_of_questions)]
    ...
    def create_question_files(self, folder: str, basename: str, question_number: str, parts: Tuple[str, ...]) -> None:
        try:
            ...
            question_filename = folder_path / \
                f"{QUESTION_PREFIX}{question_number}{TEX_EXTENSION}"
            with open(question_filename, 'w', encoding='utf-8') as file:
                file.write(question_content)
            for part in parts:
                part_filename = folder_path / \
                    f"{QUESTION_PREFIX}{question_number}{part}{TEX_EXTENSION}"
                part_content = [
    ...
    def _generate_question_content(self, basename: str, question_number: str, parts: Tuple[str, ...]) -> str:
        if not parts:
            return '\n'.join([
                ...
                "% This is an ANSWER file - EDIT THIS!",
                f"% Add your answer for Question {question_number} below.",
                "% You can use LaTeX commands, equations, figures, etc.",
                "% Generated by TMA LaTeX Generator",
                "",
                "\\begin{question}",
        ...
        for part in parts:
            ...
            lines.append(
                f"\\input{{{QUESTION_PREFIX}{question_number}{part}}}")
            ...
            for part_id, num_subparts in subparts_dict.items():
                ...
                if part_filename.exists():
                    ...
                    for i in range(num_subparts):
                        subpart_filename = folder_path / \
                            f"{part_id}_{i}{TEX_EXTENSION}"
...
class TMAGeneratorGUI:
    ...
    def _setup_styles(self) -> None:
        ...
        style.configure("Red.TLabel", foreground="red",
                        font=("Arial", 10, "bold"))
        style.configure("Orange.TLabel", foreground="#E46B0A",
                        font=("Arial", 10, "bold"))
    ...
    def _create_title_section(self, parent: ttk.Frame, row: int) -> int:
        title_label = ttk.Label(
            parent, text="TMA LaTeX Generator", font=("Arial", 16, "bold"))
    ...
    def _create_basic_settings_section(self, parent: ttk.Frame, row: int) -> int:
        settings_fields = [
            ("Course Code:", "course", 15,
             "Your module code (e.g., MATH101, PHYS201, CHEM301)", True),
            ("TMA Reference:", "tma_ref", 10,
             "TMA assignment number (e.g., 01, 02, 03, 04)", True),
            ...
            ("Title:", "title", 30,
             "Optional title for the TMA, which will appear on the title page", True),
            ("Your Name:", "name", 30,
             "Your full name as registered with your institution", True),
            ("Student PIN:", "pin", 15,
             "Your student identification number (e.g., S1234567)", True),
            ("LaTeX Style:", "style", 10,
             "Obtained from:https://ctan.org/pkg/ou-tma", False),
        ]
        ...
        self.cleveref_var = tk.BooleanVar(
            value=self.config.get("cleveref", False))
        self.pdfbookmark_var = tk.BooleanVar(
            value=self.config.get("pdfbookmark", False))
        self.legacy_var = tk.BooleanVar(value=self.config.get("legacy", False))
        options = [
            ("roman", self.roman_var,
             "Use Roman numerals for part numbering (i, ii, iii)"),
            ("cleveref", self.cleveref_var,
             "Enable cleveref for smart cross-referencing"),
            ("pdfbookmark", self.pdfbookmark_var,
             "Add PDF bookmarks for questions"),
            ("legacy", self.legacy_var,
             "Enable legacy command definitions (e.g., \\vec)")
        ]
    ...
    def _create_output_directory_field(self, parent: ttk.Frame, row: int) -> int:
        ...
        output_entry = ttk.Entry(
            parent, textvariable=self.output_var, width=30)
        output_entry.grid(row=row, column=1, sticky=(
            tk.W, tk.E), pady=ENTRY_PADY, padx=(5, 0))
        ToolTip(output_entry, "Directory where LaTeX files will be created. Use Browse button or type path directly.")
        browse_button = ttk.Button(
            parent, text="Browse", command=self._browse_output)
    ...
    def _create_question_structure_section(self, parent: ttk.Frame, row: int) -> int:
        ...
        structure_label = ttk.Label(
            parent, text="Question Structure (Manual Input)", font=("Arial", 12, "bold"))
    ...
    def _create_structure_control_buttons(self, parent: ttk.Frame, row: int) -> int:
        ...
        buttons = [
            ("Clear All", self._clear_structure,
             "Resets all fields and questions to their default values."),
            ("Add Question", self._add_question,
             "Add a new question to the structure"),
            ("Example", self._add_example_questions,
             "Populate with example questions that demonstrate the functionality"),
            ("Help", self._show_help,
             "Show comprehensive help with examples and instructions"),
        ]
        ...
        self.total_marks_label = ttk.Label(
            control_frame, text="Total Marks: 0/100")
    ...
    def _create_scrollable_question_area(self, parent: ttk.Frame, row: int) -> int:
        ...
        self.structure_scrollbar = ttk.Scrollbar(
            parent, orient="vertical", command=self.structure_canvas.yview)
        self.structure_scrollable_frame = ttk.Frame(self.structure_canvas)
        self.structure_scrollable_frame.bind("<Configure>", lambda e: self.structure_canvas.configure(
        ...
        self.structure_canvas.grid(row=row, column=0, columnspan=2, sticky=(
            tk.W, tk.E, tk.N, tk.S), pady=(5, 0))
        self.structure_scrollbar.grid(
            row=row, column=2, sticky=(tk.N, tk.S), pady=(5, 0))
        self._add_question()
        return row + 1
    ...
    def _create_action_buttons_section(self, parent: ttk.Frame, row: int) -> int:
        ...
        buttons = [
            ("Generate TMA Files", self._generate_files,
             "Create the LaTeX file structure based on your question setup"),
            ("Save Settings", self._save_settings,
             "Save your current configuration to avoid re-entering next time"),
    ...
    def _create_output_section(self, parent: ttk.Frame, row: int) -> None:
        ...
        self.output_text.grid(row=row, column=0, columnspan=3, sticky=(
            tk.W, tk.E, tk.N, tk.S), pady=(5, 10))
    ...
    def _update_total_marks(self, *args) -> None:
        ...
        self.total_marks_label.config(
            text=f"Total Marks: {total_marks}/100", style=style_name)

    def _get_numbering_maps(self, to_roman: bool):
        alph_map = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l']
        roman_map = ['i', 'ii', 'iii', 'iv', 'v', 'vi',
                     'vii', 'viii', 'ix', 'x', 'xi', 'xii']
        return (alph_map, roman_map) if to_roman else (roman_map, alph_map)

    def _convert_numbering(self, items_str: str, from_map: list, to_map: list) -> str:
        items = [p.strip() for p in items_str.split(',') if p.strip()]
        new_items = []
        for item in items:
            try:
                index = from_map.index(item)
                new_items.append(to_map[index])
            except ValueError:
    ...
    def _on_roman_toggle(self, *args):
        ...
        part_from_map, part_to_map = self._get_numbering_maps(is_roman)
        subpart_from_map, subpart_to_map = self._get_numbering_maps(not is_roman)

        for question in self.questions:
            # Convert parts
            current_parts_str = question['parts_var'].get()
            question['parts_var'].set(self._convert_numbering(
                current_parts_str, part_from_map, part_to_map))

            # Convert subparts
            current_subparts_str = question['subparts_var'].get()
            if current_subparts_str.strip():
                subpart_groups = [
                    s.strip() for s in current_subparts_str.split(';') if s.strip()]
                new_subpart_groups = []
                for group in subpart_groups:
                    ...
                    # Convert part key
                    new_part_key = self._convert_numbering(
                        part_key.strip(), part_from_map, part_to_map)

                    # Convert subpart values
                    new_sub_values = self._convert_numbering(
                        sub_values_str, subpart_from_map, subpart_to_map)

                    new_subpart_groups.append(
                        f"{new_part_key}:{new_sub_values}")
    ...
    def _refresh_question_frames(self) -> None:
        ...
        for i, question_data in enumerate(self.questions):
            ...
            question_frame = ttk.LabelFrame(
                self.structure_scrollable_frame, text=f"Question {question_num}")
    ...
    def _create_question_input_fields(self, parent: ttk.Frame, index: int, question_data: Dict[str, tk.StringVar]) -> None:
        field_defs = [
            ("Marks:", question_data['marks_var'], 5,
             "Total marks for this question (e.g., 25, 30, 15)"),
            ("Parts:", question_data['parts_var'], 20,
             "Question parts separated by commas\nExamples: 'a,b,c,d' or 'a,b' or 'a,b,c,d,e,f'"),
            ("Subparts (part:subparts):", question_data['subparts_var'], 15,
             "Subparts for each part using format: part:sub1,sub2\nExamples:\n'a:i,ii,iii' or 'a:a,b,c'\nLeave blank if no subparts"),
        ]
        ...
        add_button = ttk.Button(
            button_frame, text="Add", command=lambda i=index: self._insert_question(i + 1))
        add_button.pack(side=tk.LEFT, padx=(0, 5))
        ToolTip(add_button, "Insert a new question after this one")
        remove_button = ttk.Button(
            button_frame, text="Remove", command=lambda i=index: self._remove_question(i))
    ...
    def _clear_structure(self) -> None:
        ...
        self._add_question()
        messagebox.showinfo(
            "Clear", "All fields have been reset to their default values.")
    ...
    def _parse_subparts_string(self, subparts_text: str) -> Dict[str, Dict[str, bool]]:
        ...
        for part_subparts in subparts_text.split(';'):
            ...
            part = part.strip()
            subparts = [s.strip()
                        for s in subparts_str.split(',') if s.strip()]
    ...
    def _validate_question_structure(self) -> Optional[str]:
        ...
        for i, question_data in enumerate(self.questions):
            ...
            # Validate parts and subparts
            parts_text = question_data['parts_var'].get().strip()
            parts = [p.strip().lower()
                     for p in parts_text.split(',') if p.strip()]
            subparts_text = question_data['subparts_var'].get().strip()

            error = self._validate_parts(question_num, parts, subparts_text)
            if error:
                return error

            error = self._validate_subparts(question_num, parts, subparts_text)
            if error:
                return error
    ...
    def _validate_parts(self, question_num, parts, subparts_text):
        if not parts and subparts_text:
            return f"Question {question_num}: Cannot have subparts without parts."
    ...
    def _validate_subparts(self, question_num, parts, subparts_text):
        if not subparts_text:
            return None
    ...
    def _handle_marks_total_mismatch(self, total_marks: int) -> Optional[str]:
        ...
        continue_anyway = messagebox.askyesno(
            "Marks Total Warning", message, icon='warning')
        if continue_anyway:
            return None
        else:
    ...
    def _generate_files(self) -> None:
        ...
        try:
            ...
            if success:
                ...
                messagebox.showinfo(
                    "Success", "TMA files generated successfully!")
            else:
    ...
    def _generate_tma_files(self, config: Dict[str, str], structure: Dict[str, Dict[str, Union[int, Dict[str, Dict[str, bool]]]]]) -> Tuple[bool, str]:
        try:
            ...
            self.output_text.insert(
                tk.END, "Using manual question structure...\n")
            self.output_text.see(tk.END)
            self.output_text.update()
            self._display_structure_summary(structure)
            parts_list, subparts_dict = self._prepare_generation_data(
                structure)
            actual_folder = generator.create_directory(config["output"])
            self.output_text.insert(
                tk.END, f"Created directory: {actual_folder}\n")
            self.output_text.see(tk.END)
            self.output_text.update()
            generator.create_main_tex_file(
                actual_folder, config["basename"], len(structure))
            for i, question_parts in enumerate(parts_list):
                generator.create_question_files(
                    actual_folder, config["basename"], str(i + 1), question_parts)
            if subparts_dict:
                generator.create_subparts(
                    actual_folder, config["basename"], subparts_dict)
            copied_styles = generator.copy_style_files(actual_folder)
            if copied_styles:
                self.output_text.insert(
                    tk.END, f"Copied style files: {', '.join(copied_styles)}\n")
                self.output_text.see(tk.END)
                self.output_text.update()
            ...
            self.output_text.insert(
                tk.END, f"Suggested Overleaf project name:\n")
            self.output_text.insert(tk.END, f"  {suggested_name}\n\n")
            self.output_text.insert(tk.END, "Next steps:\n")
            self.output_text.insert(
                tk.END, "1. Create new blank project in Overleaf\n")
            self.output_text.insert(
                tk.END, "2. Use the suggested name above\n")
            self.output_text.insert(
                tk.END, "3. Delete default main.tex in Overleaf\n")
            self.output_text.insert(
                tk.END, "4. Upload ALL files from output directory\n")
            self.output_text.insert(
                tk.END, "5. Compile and start editing!\n\n")
            self.output_text.insert(
                tk.END, "Generation completed successfully!\n")
            self.output_text.see(tk.END)
    ...
    def _prepare_generation_data(self, structure: Dict[str, Dict[str, Union[int, Dict[str, Dict[str, bool]]]]]) -> Tuple[List[Tuple[str, ...]], Dict[str, int]]:
        ...
        for q_id in sorted(structure.keys(), key=lambda x: int(x[1:])):
            ...
            for part_id, part_data in q_data.get('parts', {}).items():
                ...
                if subparts:
                    subparts_dict[f"{QUESTION_PREFIX}{q_id[1:]}{part_id}"] = len(
                        subparts)
