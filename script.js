...
const Validation = {
    ...
    validateQuestions(questions) {
        ...
        questions.forEach((question, index) => {
            ...
            if (parts.length === 0 && question.subparts.trim() !== '') {
                errors.push(`Question ${qNum}: Cannot have subparts without parts.`);
            }
...
const LaTeXGenerator = {
    ...
    generateMainTex(config, questions) {
        ...
        if (config.cleveref) options.push('cleveref');
        if (config.pdfbookmark) options.push('pdfbookmark');
        if (config.legacy) options.push('legacy');

        const usepackage = options.length > 0 ? `\\usepackage[${options.join(',')}]` : '\\usepackage';
        ...
        const lines = [
            ...
            `${usepackage}{${config.style.replace('.sty', '')}}`,
            `\\myname{${config.name}}`,
            `\\mypin{${config.pin}}`,
    ...
    generateQuestionTex(config, questionIndex, parts) {
        ...
        if (parts.length === 0) {
            // This is an answer file for a question with no parts.
            return [
                `% !TeX root = ./${config.basename}.tex`,
                `% File: q${qNum}.tex`,
                '% This is an ANSWER file - EDIT THIS!',
                `% Add your answer for Question ${qNum} below.`,
...
const UI = {
    ...
    cacheElements() {
        this.elements = {
            ...
            cleveref: document.getElementById('cleveref'),
            pdfbookmark: document.getElementById('pdfbookmark'),
            ...
            clearSavedSettings: document.getElementById('clear-saved-settings'),
    ...
    bindEvents() {
        ...
        this.elements.addExample.addEventListener('click', () => this.addExampleQuestions());
        ...
        this.elements.exportSettings.addEventListener('click', () => Storage.exportSettings());
        this.elements.clearSavedSettings.addEventListener('click', () => this.clearSavedSettings());
        ...
        Object.keys(State.currentConfig).forEach(key => {
            if (this.elements[key]) {
                if (this.elements[key].type === 'checkbox') {
                    this.elements[key].checked = State.currentConfig[key];
                } else {
    ...
    updateConfig() {
        ...
        State.currentConfig['roman'] = this.elements.roman.checked;
        State.currentConfig['cleveref'] = this.elements.cleveref.checked;
        State.currentConfig['pdfbookmark'] = this.elements.pdfbookmark.checked;
        State.currentConfig['legacy'] = this.elements.legacy.checked;
    },
    ...
    clearAllQuestions() {
        if (State.questions.length > 0) {
            if (!confirm("This will clear all current questions and reset all fields. Are you sure?")) {
                return;
            }
        }

        // Reset config and form fields to default
        State.currentConfig = { ...CONFIG.DEFAULT_VALUES };
        this.updateFormFields();

        // Clear all questions and add one back
        State.questions = [];
        this.addQuestion();
        
        Utils.showNotification('All fields have been reset to their default values.', 'info');
    },
    ...
    renderQuestion(question, index) {
        ...
        questionDiv.innerHTML = `
            <div class="question-header">
                ...
                <div class="question-buttons">
                    <button type="button" class="btn btn-primary btn-small add-after-question" data-tooltip="Add a new question after this one">
                        ➕ Add
                    </button>
                    <button type="button" class="btn btn-danger btn-small remove-question" data-tooltip="Remove this question">
                        🗑️ Remove
                    </button>
                </div>
            </div>
        ...
        const addBtn = questionDiv.querySelector('.add-after-question');
    ...
    showHelp() {
        ...
        addExampleQuestions() {
            if (State.questions.length > 0) {
                if (!confirm("This will clear all current questions. Are you sure?")) {
                    return;
                }
            }
            State.questions = []; // Clear existing questions

            const use_roman = this.elements.roman.checked;

            let exampleQuestions;
            if (use_roman) {
                State.currentConfig.course = 'M381';
                this.updateFormFields();
                exampleQuestions = [
            ...
            exampleQuestions.forEach(q_data => {
                const question = {
                    id: Utils.generateId(),
                    marks: q_data.marks,
                    parts: q_data.parts,
                    subparts: q_data.subparts
                };
                State.questions.push(question);
            });
    ...
    getHelpContent() {
        return `
            <div class="help-content">
                ...
                <ul>
                    <li><strong>Course Code:</strong> Your module code (e.g., M101, PHYS201)</li>
                    ...
                    <li><strong>Your Name:</strong> Your full name as registered (e.g., Alex Noel Other)</li>
                    <li><strong>Student PIN:</strong> Your student identification number</li>
                    <li><strong>LaTeX Style:</strong> The LaTeX style file to use. This is set to 'ou-tma' and cannot be changed.</li>
                ...
                <h4>3. Package Options</h4>
                <ul>
                    <li><strong>roman:</strong> Use Roman numerals for part numbering (i, ii, iii). If not selected, alphabetic numbering (a, b, c) is used.</li>
                    <li><strong>cleveref:</strong> Enable cleveref for smart cross-referencing.</li>
                    <li><strong>pdfbookmark:</strong> Add PDF bookmarks for questions.</li>
                    <li><strong>legacy:</strong> Enable legacy command definitions (e.g., \\vec).</li>
                </ul>

                <h4>4. Buttons</h4>
                <ul>
                    <li><strong>Add Question:</strong> Creates a new question entry.</li>
                    <li><strong>Clear All:</strong> Resets all fields and questions to their default values.</li>
                    <li><strong>Example:</strong> Populates the form with example questions.</li>
                    <li><strong>Help:</strong> Shows this help dialog.</li>
                    <li><strong>Generate TMA Files:</strong> Creates the LaTeX file structure and downloads it as a ZIP file.</li>
                    <li><strong>Save Settings:</strong> Saves your current configuration to your browser's local storage.</li>
                    <li><strong>Load Settings:</strong> Loads a previously saved configuration from a .json file.</li>
                    <li><strong>Export Settings:</strong> Exports your current configuration to a .json file.</li>
                    <li><strong>Clear Saved Settings:</strong> Deletes your saved settings from your browser's local storage and reloads the application with the default settings.</li>
                </ul>

                <h4>5. Using with Overleaf</h4>
    ...
    clearSavedSettings() {
        if (confirm("This will permanently delete your saved settings from this browser. Are you sure?")) {
            localStorage.removeItem(CONFIG.STORAGE_KEYS.SETTINGS);
            localStorage.removeItem(CONFIG.STORAGE_KEYS.QUESTIONS);
            location.reload();
        }
    },
